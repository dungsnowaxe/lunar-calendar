...
function mockWriter(options?: { error?: string; rows?: EventRow[] }): {
  supabase: MemorialEventsWriter;
  inserts: MemorialEventInsert[][];
} {
...
test("one invalid entry creates none of the batch", async () => {
  const { supabase, inserts } = mockWriter();
  const result = await createMemorialEvents([VALID_ONG, { ...VALID_BA, title: "   " }], supabase);
