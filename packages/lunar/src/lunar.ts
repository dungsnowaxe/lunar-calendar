import { getLunarMonth11, getLeapMonthOffset, jdToLunar, lunarToJd } from "./hnd.ts";
...
export function lunarMonthLength(lunarYear: number, month: number, isLeapMonth: boolean): number {
  ...
  if (check.month !== month || check.year !== lunarYear || (check.leap === 1) !== isLeapMonth) {
    return 0;
  }
