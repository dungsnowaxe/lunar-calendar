...
test('Tết Nguyên Đán matches published Vietnamese dates 2015–2026', () => {
  for (const [lunarYear, expected] of TET) {
    ...
    assert.deepEqual(solarToLunar(expected), {
      day: 1,
      month: 1,
      year: lunarYear,
      isLeapMonth: false,
...
test('tháng 6 nhuận 2025 runs 25/07–22/08 and has 29 days', () => {
  ...
  assert.deepEqual(
    lunarToSolar({ day: 1, month: 6, year: 2025, isLeapMonth: true }),
    first,
  )
...
test('invalid lunar dates return null', () => {
  ...
  assert.equal(lunarToSolar({ day: 1, month: 5, year: 2026, isLeapMonth: true }), null,
